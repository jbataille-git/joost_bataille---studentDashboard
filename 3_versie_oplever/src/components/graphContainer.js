...
import { VictoryChart, VictoryBar, VictoryTheme, VictoryAxis, VictoryGroup, VictoryLabel } from 'victory';
...
const GraphContainer = () => {
  ...
  // --------------------------------------------------------------------------
  let studentData = useSelector(state => state.studentData.studentData);
  ...
  const assignmentArray = useSelector(state => state.studentData.assignmentArray);
  ...
  assignmentArray.forEach( a => {
    ...
    const sumDif = selectedStudentData
      ...
      .reduce((currentTotal, assignment) =>  currentTotal + parseInt(assignment.difficulty), 0);
    ...
    const sumJoy = selectedStudentData
      ...
      .reduce((currentTotal, assignment) =>  currentTotal + parseInt(assignment.enjoyment), 0);

    // watch out for division by zero 
    let result = {
      assignment: a,
      resultDif: (numberOfSelectedStudents === 0 ? 0 : sumDif/numberOfSelectedStudents),
      resultJoy: (numberOfSelectedStudents === 0 ? 0 : sumJoy/numberOfSelectedStudents)
    }
        ...
        />
          ...
          >
            <VictoryBar
              ...
              data = {resultArray}
              x="assignment"
              y={ selectButtons.dif ? "resultDif" : null }
            /> 
            <VictoryBar
              ...
              data = {resultArray}
              x="assignment"
              y={ selectButtons.joy ? "resultJoy" : null }
            />
          </VictoryGroup>
      </VictoryChart>
      { graphSelectButtons }
